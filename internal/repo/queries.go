...
func (r *Repo) CreateUser(ctx context.Context, email, passHash string) (int64, error) {
	row := r.DB.QueryRowContext(ctx, `INSERT INTO users(email, pass_hash, role) VALUES ($1,$2,'student') RETURNING id`, email, passHash)
	var id int64
	return id, row.Scan(&id)
}
func (r *Repo) FindUserByEmail(ctx context.Context, email string) (*UserRow, error) {
	row := r.DB.QueryRowContext(ctx, `SELECT id, email, pass_hash, role FROM users WHERE email=$1`, email)
	var u UserRow
	if err := row.Scan(&u.ID, &u.Email, &u.PassHash, &u.Role); err != nil { return nil, err }
	return &u, nil
}
func (r *Repo) UpdateUserPass(ctx context.Context, userID int64, newHash string) error {
	_, err := r.DB.ExecContext(ctx, `UPDATE users SET pass_hash=$2 WHERE id=$1`, userID, newHash)
	return err
}
...
func (r *Repo) ListUsers(ctx context.Context) ([]UserRow, error) {
	rows, err := r.DB.QueryContext(ctx, `SELECT id,email,pass_hash,role FROM users ORDER BY id`)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []UserRow
	for rows.Next() {
		var u UserRow
		if err := rows.Scan(&u.ID, &u.Email, &u.PassHash, &u.Role); err != nil { return nil, err }
		out = append(out, u)
	}
	return out, rows.Err()
}
func (r *Repo) UpdateUserRole(ctx context.Context, userID int64, role string) error {
	switch role { case "student","teacher","admin": default: return fmt.Errorf("invalid role") }
...
func (r *Repo) ListCourses(ctx context.Context) ([]CourseRow, error) {
	rows, err := r.DB.QueryContext(ctx, `SELECT id,title,description FROM courses ORDER BY id`)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []CourseRow
	for rows.Next() {
		var c CourseRow
		if err := rows.Scan(&c.ID, &c.Title, &c.Description); err != nil { return nil, err }
		out = append(out, c)
	}
	return out, rows.Err()
}
func (r *Repo) CreateCourse(ctx context.Context, title, description string) error {
	_, err := r.DB.ExecContext(ctx, `INSERT INTO courses(title, description) VALUES ($1,$2)`, title, description)
	return err
}
func (r *Repo) UpdateCourse(ctx context.Context, id int64, title, description string) error {
	_, err := r.DB.ExecContext(ctx, `UPDATE courses SET title=COALESCE(NULLIF($2,''),title), description=COALESCE(NULLIF($3,''),description) WHERE id=$1`, id, title, description)
	return err
}
...
func (r *Repo) LoadQuizRules(ctx context.Context, quizID int64) (*QuizRules, string, error) {
	var rulesRaw []byte
	var title string
	err := r.DB.QueryRowContext(ctx, `SELECT rules, title FROM quizzes WHERE id=$1`, quizID).Scan(&rulesRaw, &title)
	if err != nil { return nil, "", err }
	var q QuizRules
	if len(rulesRaw) > 0 {
		if err := json.Unmarshal(rulesRaw, &q); err != nil { return nil, "", err }
	}
	if q.Count == 0 { q.Count = 10 }
	return &q, title, nil
}
func (r *Repo) ListQuizzesByCourse(ctx context.Context, courseID int64) ([]QuizRow, error) {
	rows, err := r.DB.QueryContext(ctx, `SELECT id, title, rules FROM quizzes WHERE course_id=$1 ORDER BY id`, courseID)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []QuizRow
	for rows.Next() {
		var q QuizRow
		if err := rows.Scan(&q.ID, &q.Title, &q.Rules); err != nil { return nil, err }
		out = append(out, q)
	}
	return out, rows.Err()
}
func (r *Repo) CreateQuiz(ctx context.Context, courseID int64, title string, rulesRaw []byte) error {
	...
	if err := json.Unmarshal(rulesRaw, &tmp); err != nil { return fmt.Errorf("invalid JSON in rules: %w", err) }
	_, err := r.DB.ExecContext(ctx, `INSERT INTO quizzes(course_id, title, rules) VALUES ($1,$2,$3)`, courseID, title, rulesRaw)
	return err
}
func (r *Repo) DeleteQuiz(ctx context.Context, quizID int64) error {
	_, err := r.DB.ExecContext(ctx, `DELETE FROM quizzes WHERE id=$1`, quizID)
	return err
}
...
func (r *Repo) PickQuestions(ctx context.Context, courseID int64, rules *QuizRules) ([]QuestionRow, error) {
	var rows *sql.Rows
	var err error
	if len(rules.ByTopics) > 0 {
		rows, err = r.DB.QueryContext(ctx, `
			SELECT id, topic, q_type, difficulty, payload
			FROM questions
			WHERE course_id=$1 AND topic = ANY($2)
			ORDER BY random()
			LIMIT $3
		`, courseID, toPGTextArray(rules.ByTopics), rules.Count)
	} else {
		rows, err = r.DB.QueryContext(ctx, `
			SELECT id, topic, q_type, difficulty, payload
			FROM questions
	...
	for rows.Next() {
		var q QuestionRow
		if err := rows.Scan(&q.ID, &q.Topic, &q.QType, &q.Difficulty, &q.Payload); err != nil { return nil, err }
		out = append(out, q)
	}
	return out, rows.Err()
}
...
func (r *Repo) FetchQuestionsByIDs(ctx context.Context, ids []int64) ([]QuestionRow, error) {
	...
	for i, id := range ids { ph[i] = fmt.Sprintf("$%d", i+1); args[i] = id }
	q := `SELECT id, topic, q_type, difficulty, payload FROM questions WHERE id IN (` + strings.Join(ph, ",") + `)`
	...
	for rows.Next() {
		var v QuestionRow
		if err := rows.Scan(&v.ID, &v.Topic, &v.QType, &v.Difficulty, &v.Payload); err != nil { return nil, err }
		out = append(out, v)
	}
	return out, rows.Err()
}
...
func (r *Repo) ListQuestions(ctx context.Context, courseID int64, topic, qtype string, limit int) ([]QuestionRow, error) {
	...
	args := []any{courseID}
	i := 2
	sb.WriteString(`SELECT id, topic, q_type, difficulty, payload FROM questions WHERE course_id=$1`)
	if topic != "" { sb.WriteString(fmt.Sprintf(" AND topic=$%d", i)); args = append(args, topic); i++ }
	if qtype != "" { sb.WriteString(fmt.Sprintf(" AND q_type=$%d", i)); args = append(args, qtype); i++ }
...
func (r *Repo) GetQuestion(ctx context.Context, id int64) (*QuestionRow, error) {
	row := r.DB.QueryRowContext(ctx, `SELECT id, topic, q_type, difficulty, payload FROM questions WHERE id=$1`, id)
	var q QuestionRow
	if err := row.Scan(&q.ID, &q.Topic, &q.QType, &q.Difficulty, &q.Payload); err != nil { return nil, err }
	return &q, nil
}
func (r *Repo) UpdateQuestion(ctx context.Context, id int64, topic, qtype string, diff int, payload []byte) error {
	if qtype != "" {
		switch qtype { case "single","multiple","numeric","text": default: return fmt.Errorf("invalid q_type") }
	}
	if diff != 0 && (diff < 1 || diff > 5) { return fmt.Errorf("invalid difficulty") }
	_, err := r.DB.ExecContext(ctx, `
		UPDATE questions
		   SET topic      = COALESCE(NULLIF($2,''), topic),
		       q_type     = COALESCE(NULLIF($3,''), q_type),
...
func (r *Repo) CreateAttempt(ctx context.Context, quizID, userID int64) (int64, error) {
	var id int64
	err := r.DB.QueryRowContext(ctx, `INSERT INTO attempts(quiz_id, user_id) VALUES ($1,$2) RETURNING id`, quizID, userID).Scan(&id)
	return id, err
}
func (r *Repo) SaveAnswer(ctx context.Context, attemptID, questionID int64, isCorrect *bool, answer []byte) error {
	_, err := r.DB.ExecContext(ctx, `INSERT INTO answers(attempt_id, question_id, is_correct, answer) VALUES ($1,$2,$3,$4)`, attemptID, questionID, isCorrect, answer)
	return err
}
func (r *Repo) SetAttemptResult(ctx context.Context, attemptID int64, finishedAt *time.Time, score *float64) error {
	_, err := r.DB.ExecContext(ctx, `UPDATE attempts SET finished_at=$2, total_score=$3 WHERE id=$1`, attemptID, finishedAt, score)
	return err
}
func (r *Repo) SetAttemptTiming(ctx context.Context, attemptID int64, durationSec int, overtime bool) error {
	_, err := r.DB.ExecContext(ctx, `UPDATE attempts SET duration_sec=$2, overtime=$3 WHERE id=$1`, attemptID, durationSec, overtime)
	return err
}
...
func (r *Repo) ListAttemptsByCourse(ctx context.Context, courseID int64) ([]AttemptRow, error) {
	...
	for rows.Next() {
		var r0 AttemptRow
		if err := rows.Scan(&r0.ID, &r0.UserEmail, &r0.QuizTitle, &r0.FinishedAt, &r0.Score); err != nil { return nil, err }
		out = append(out, r0)
	}
	return out, rows.Err()
}
...
func (r *Repo) GetAttemptWithAnswers(ctx context.Context, attemptID int64) (*AttemptMeta, []AnswerDetail, error) {
	...
	err := r.DB.QueryRowContext(ctx, `
		SELECT a.id, u.email, qz.title, a.started_at, a.finished_at, a.total_score, a.duration_sec, a.overtime
		FROM attempts a
		JOIN users u ON u.id=a.user_id
		JOIN quizzes qz ON qz.id=a.quiz_id
		WHERE a.id=$1
	`, attemptID).Scan(&meta.ID, &meta.UserEmail, &meta.QuizTitle, &meta.StartedAt, &meta.FinishedAt, &meta.Score, &meta.DurationSec, &meta.Overtime)
	if err != nil { return nil, nil, err }

	rows, err := r.DB.QueryContext(ctx, `
		SELECT q.id, q.topic, q.q_type, q.payload, an.is_correct, an.answer
		FROM answers an
	...
	for rows.Next() {
		var d AnswerDetail
		if err := rows.Scan(&d.QuestionID, &d.Topic, &d.QType, &d.Payload, &d.IsCorrect, &d.Answer); err != nil { return nil, nil, err }
		out = append(out, d)
	}
...
func (r *Repo) TotalAttemptsByUserQuiz(ctx context.Context, userID, quizID int64) (int, error) {
	var n int
	err := r.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM attempts WHERE user_id=$1 AND quiz_id=$2`, userID, quizID).Scan(&n)
	return n, err
}
...
func (r *Repo) ExportAttempts(ctx context.Context, courseID *int64, quizID *int64) ([]AttemptExportRow, error) {
	...
	sb.WriteString(`
		SELECT a.id, u.email, q.course_id, q.id, q.title, a.started_at, a.finished_at, a.total_score, a.duration_sec, a.overtime
		FROM attempts a
		JOIN users u ON u.id=a.user_id
	...
	if courseID != nil {
		...
		args = append(args, *courseID); i++
	}
	if quizID != nil {
		where = append(where, fmt.Sprintf("q.id=$%d", i))
		args = append(args, *quizID); i++
	}
	if len(where) > 0 {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r0.AttemptID, &r0.UserEmail, &r0.CourseID, &r0.QuizID, &r0.QuizTitle, &r0.StartedAt, &r0.FinishedAt, &r0.Score, &r0.Duration, &r0.Overtime); err != nil {
			return nil, err
		}
		out = append(out, r0)
	}
...
func (r *Repo) TopicStatsByUser(ctx context.Context, userID, courseID int64) ([]TopicStat, error) {
	...
	for rows.Next() {
		var s TopicStat
		if err := rows.Scan(&s.Topic, &s.Total, &s.Correct); err != nil { return nil, err }
		out = append(out, s)
	}
	return out, rows.Err()
}
...
func (r *Repo) TopicDetail(ctx context.Context, userID, courseID int64, topic string) ([]TopicDetailRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r0.QID, &r0.When, &r0.Correct); err != nil { return nil, err }
		out = append(out, r0)
	}
	return out, rows.Err()
}
...
func (r *Repo) ImportQuestionsCSV(ctx context.Context, reader *csv.Reader, courseID int64) (int, error) {
	...
	for {
		...
		if len(rec) < 6 { return count, fmt.Errorf("invalid record length: %v", rec) }
		...
		choicesRaw := ""
		correctRaw := ""
		if len(rec) > 3 { choicesRaw = strings.TrimSpace(rec[3]) }
		if len(rec) > 4 { correctRaw = strings.TrimSpace(rec[4]) }
		diffStr := strings.TrimSpace(rec[5])

		diff := 3
		if v, err := strconv.Atoi(diffStr); err == nil { diff = v }
		...
		case "single":
			...
			payload = map[string]any{ "text": qtext, "choices": choices, "correct": []int{corrIdx} }
		case "multiple":
			choices := splitComma(choicesRaw)
			var corr []int
			for _, p := range splitComma(correctRaw) { if i,err:=strconv.Atoi(p); err==nil { corr = append(corr,i) } }
			payload = map[string]any{ "text": qtext, "choices": choices, "correct": corr }
		case "numeric":
			val, _ := strconv.ParseFloat(strings.ReplaceAll(correctRaw, ",","."), 64)
			payload = map[string]any{ "text": qtext, "correct_value": val }
		case "text":
			payload = map[string]any{ "text": qtext, "accept": splitComma(correctRaw) }
		default:
			return count, fmt.Errorf("unsupported q_type: %s", qtype)
		}
		raw, _ := json.Marshal(payload)
		if _, err := r.DB.ExecContext(ctx,
			`INSERT INTO questions(course_id, topic, difficulty, q_type, payload) VALUES ($1,$2,$3,$4,$5)`,
...
// JSON массив объектов: { "topic","q_type","difficulty", "payload":{...} }
func (r *Repo) ImportQuestionsJSON(ctx context.Context, raw []byte, courseID int64) (int, error) {
	...
	for _, it := range items {
		...
		if it.Difficulty == 0 { it.Difficulty = 3 }
		if _, err := r.DB.ExecContext(ctx,
			`INSERT INTO questions(course_id, topic, difficulty, q_type, payload) VALUES ($1,$2,$3,$4,$5)`,
...
func (r *Repo) UserLogs(ctx context.Context, userID int64) (*UserLogSummary, []UserLogRow, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			// Пользователь есть, но активностей нет — вернём пустую сводку
